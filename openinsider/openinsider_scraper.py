...
Record = namedtuple("record",
                    ['x', 'filing_date', 'trade_date', 'ticker', 'company',
                     'insider', 'title', 'trade_type', 'price', 'qty',
                     'owned', 'new_own', 'value', 'one_d', 'one_w',
                     'one_m', 'six_m'])
FIELD_NAMES = Record('X', 'Filing\xa0Date', 'Trade\xa0Date', 'Ticker',
                     'Company\xa0Name', 'Insider\xa0Name', 'Title', 'Trade\xa0Type',
                     'Price', 'Qty', 'Owned', 'ΔOwn',
                     'Value', '1d', '1w', '1m', '6m')
...
class OpenInsiderScraper:
    ...
    def _get_data_for_month(self, year: int, month: int) -> Set[tuple]:
        ...
        try:
            ...
            field_names = [h.text.strip() for h in headers]
            if field_names != list(FIELD_NAMES):
                self.logger.error(f"Header fields do not match {field_names}")
                return set()
            ...
            for row in rows:
                ...
                insider_data = Record(*(col.find('a').text.strip() if col.find('a') else col.text.strip() for col in cols))

                # Apply filters
                if self._is_keep(insider_data):
                    data.add(tuple(insider_data))
    ...
    def _is_keep(self, data: Record) -> bool:
        try:
    ...
    def _save_data(self, data: List[tuple]) -> None:
        df = pd.DataFrame(data, columns=list(Record._fields))
